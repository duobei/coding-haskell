...
rev :: SKI (a -> (a -> b) -> b)
rev = S `Ap` (K `Ap` (S `Ap` I)) `Ap` (S `Ap` (K `Ap` K) `Ap` I)

comp :: SKI ((b -> c) -> (a -> b) -> (a -> c))
comp = S `Ap` (K `Ap` S) `Ap` K

flip' :: SKI ((a -> b -> c) -> (b -> a -> c))
flip' = S `Ap` (S `Ap` (K `Ap` (S `Ap` (K `Ap` S) `Ap` K)) `Ap` S) `Ap` (K `Ap` K)

rotr :: SKI (a -> (c -> a -> b) -> c -> b)
rotr = comp `Ap` comp `Ap` rev
-- \x f z -> f z x

rotv :: SKI (a -> b -> (a -> b -> c) -> c)
rotv = comp `Ap` (rotr `Ap` rotr `Ap` comp) `Ap` rotr
-- \x y f -> f x y
...
xor' = comp `Ap` join `Ap` (rotr `Ap` not' `Ap` comp)